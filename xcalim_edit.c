...
Boolean        FoundCalendarDir; /* whether the Calendar directory exists */
    ...
    Cardinal        mo;
{
    ...
    /* so let's look here for the any files of interest */
    dir = ".";
    inSubDir = False;
    if (mw->mw_dir) {
        dir = mw->mw_dir;
...
SetAllButtons(ins, day, val)
    ...
    Boolean         val;
{
    for (; ins; ins = ins->i_next) XtSetSensitive(ins->i_day_info[day], val);
}
...
/* Leave without saving the data? Let us see if the user really wants to */
static void
CheckExit(ed)
    register EditLine *ed;
{
    ...
    String      txt;
{
    ...
    /* shouldn`t happen */
    if ((mw = MeWrapSearch(da->year, da->month)) == NULL) mw = NewMeWrap();

    if (EditCheck(w, mw, da)) return;
